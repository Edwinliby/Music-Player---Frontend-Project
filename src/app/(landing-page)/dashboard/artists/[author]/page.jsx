...
import { useMusicPlayer } from '@/context/MusicPlayerContext' // ✅ context import
...
export default function ArtistPage() {
    ...
    const { currentIndex, setCurrentIndex, setIsPlaying } = useMusicPlayer()
                ...
                <div className="custom-scrollbar h-full">
                    <div className="relative">
                        ...
                        <div className="absolute bottom-8 inset-x-4 md:inset-x-8 text-white font-semibold">
                            <p>Public Playlist</p>
                            <h1 className="font-bold text-5xl xl:text-[5rem] md:leading-14 xl:leading-26 py-2 xl:py-0">
                                This Is {author}
                            </h1>
                            <p className="text-sm text-white/70">
                                The essential tracks from {author}.
                            </p>
                            <div className="flex items-center flex-wrap gap-2 mt-2">
                                <Image src={Logo} alt="Logo" width={30} height={30} />
                                <p>Wave Player •</p>
                                <p className="text-sm text-white/70">
                                    {hear} • {subSongs.length} songs, about 54 mins
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col py-4 md:py-6">
                        <div className="sticky top-0 left-0 text-gray-600 flex items-center py-2 border-b border-gray-200 px-4 md:px-6 bg-white z-10">
                            <span className="w-8 text-left">#</span>
                            <span className="flex-1 text-left">Title</span>
                            <span className="hidden md:block pr-10 lg:pr-0 lg:flex-1 text-left">
                                Album
                            </span>
                            <span className="hidden md:block pr-10 lg:pr-0 lg:flex-1 text-left">
                                Date added
                            ...
                            >
                                <span className="w-8 text-left">
                                    {hoveredIndex === index ? (
                                        <Play
                                            size={20}
                                            className="fill-gray-500 text-gray-500 relative -left-1.5"
                                        />
                                    ) : (
                                        index + 1
                                    )}
                                ...
                                <span className="hidden md:block pr-8 lg:pr-0 lg:flex-1 text-sm text-left pl-1">
                                    {song.album}
                                </span>
                                <span className="hidden md:block pr-8 lg:pr-0 lg:flex-1 text-sm text-left">
                                    {song.date}
                                </span>
